  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex items-center gap-3 bg-secondary/80 px-4 py-2 rounded-full border border-border">
        <RefreshCw
          className={`w-4 h-4 text-primary ${countdown === 15 ? "animate-spin" : ""}`}
        />
        <span className="text-sm font-medium text-foreground tracking-wide">
          QR refreshes in{" "}
          <span className="text-primary w-4 inline-block text-center">
            {countdown}
          </span>
          s
        </span>
      ...
      <p className="text-xs text-muted-foreground mt-4 max-w-xs text-center">
        This is a cryptographic rotating QR code. Screenshots will not work for
        entry.
      </p>
    </div>
  );
