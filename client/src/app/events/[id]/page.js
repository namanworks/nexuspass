...
export default function EventDetailPage({ params }) {
  ...
  const handleSeatClick = async (slot) => {
    ...
    try {
      ...
      setBookingError(
        err.message || "Failed to reserve seat. It may have just been taken.",
      );
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-grow">
              ...
              <p className="text-muted-foreground mb-4">
                Share this link with your friends so they can join and pay for
                their own seats.
              </p>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-4 text-sm font-medium">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-red-500/50" /> Sold
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-8 overflow-x-auto">
          ...
          <div className="w-full max-w-2xl mx-auto h-8 bg-gradient-to-b from-primary/30 to-transparent rounded-t-full mb-12 flex items-center justify-center border-t border-primary/50 shadow-[0_0_20px_rgba(59,130,246,0.3)]">
            <span className="text-primary font-bold uppercase tracking-widest text-sm opacity-80">
              Stage
            </span>
          </div>
          ...
          <div className="grid grid-cols-5 md:grid-cols-10 gap-3 min-w-max pb-4">
            {slots.map((slot) => {
              let seatStyles =
                "bg-green-500/20 text-green-400 border-green-500/50 hover:bg-green-500/40 hover:scale-110 cursor-pointer shadow-[0_0_10px_rgba(34,197,94,0.1)]";

              if (slot.status === "locked") {
                seatStyles =
                  "bg-orange-500/20 text-orange-400 border-orange-500/50 cursor-not-allowed opacity-80 shadow-[0_0_10px_rgba(249,115,22,0.1)]";
              } else if (slot.status === "sold") {
                seatStyles =
                  "bg-red-500/10 text-red-500/50 border-red-500/20 cursor-not-allowed opacity-50";
              }
