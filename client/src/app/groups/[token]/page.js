...
export default function GroupPage({ params }) {
  ...
  useEffect(() => {
    async function joinAndLoad() {
      try {
        ...
        const data = await fetchApi(`/groups/join/${token}`, {
          method: "POST",
        });
        setGroup(data);
      } catch (err) {
  ...
  const mySeat = group.members.find(
    (m) => m.userId === currentUser?.userId && m.seatLabel,
  );
  const isLeader = currentUser?.userId === group.leaderUserId;
  const unpaidSeatedMembers = group.members.filter(
    (m) => m.seatLabel && m.paymentStatus === "pending",
  );
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 pb-6 border-b border-border gap-4">
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2">
              Group Members{" "}
              <span className="px-3 py-1 bg-secondary text-muted-foreground rounded-full text-xs font-medium border border-border ml-2">
        ...
        <div className="flex flex-col gap-4">
          {group.members.map((member, index) => {
            ...
            return (
              <div
                key={index}
                className={`flex items-center justify-between p-4 rounded-lg border transition-all ${isMe ? "bg-primary/5 border-primary/30" : "bg-secondary/30 border-border"}`}
              >
          ...
          <div className="mt-8 p-4 bg-orange-500/10 border border-orange-500/30 rounded-lg flex items-start gap-4">
            ...
            <div>
              <p className="font-medium text-orange-400">
                You haven't selected a seat yet.
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Please return to the event page to select and lock a seat for
                yourself within this group.
              </p>
              <Link
                href={`/events/${group.eventId}?groupId=${group.groupId}`}
                className="inline-block mt-3 px-4 py-2 bg-secondary hover:bg-secondary/80 text-foreground text-sm font-medium rounded-md transition-colors"
              >
                Go to Event Page
              </Link>
            </div>
          ...
          <div className="mt-8 p-6 bg-primary/10 border border-primary/30 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-6">
            <div>
              <h3 className="text-xl font-bold">
                Pay for {unpaidSeatedMembers.length} Seat
                {unpaidSeatedMembers.length > 1 ? "s" : ""}
              </h3>
